...
// Workaround to ensure tests run in order since they depend on shared state (cart/checkout)
test.describe.configure({ mode: 'serial' });

test.describe("Checkout flow", () => {
  test("should allow a user to fill out the form and place an order", async ({
    page,
    request,
  }) => {
  ...
  test("shows empty cart state when cart has been checked out", async ({
    page,
    request,
  }) => {
    await request.post("/api/cart/random");
  ...
  test("handles duplicate submission gracefully with 409 error", async ({
    page,
    request,
  }) => {
    await request.post("/api/cart/random");
    ...
    await expect(
      page.getByText("This order has already been placed"),
    ).toBeVisible();
  });

  test("layout does not overflow on mobile viewport (375px)", async ({
    page,
    request,
  }) => {
    await request.post("/api/cart/random");
